...
- (BOOL)cancel:(nullable id)token {
    ...
    dispatch_barrier_sync(self.barrierQueue, ^{
        [self.callbackBlocks removeAllObjects]; //已经移除了全部还有一个判断是为何啊????
...
- (void)start {
    ...
    if (self.dataTask) {
        ...
        //MCDownloadStartNotification 需要发送通知
    } else {
...
- (void)cancelInternal {
    ...
    if (self.dataTask) {
        ...
        [self.receipt setState:MCDownloadStateNone];
         //MCDownloadStopNotification 需要发送通知
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    if (![response respondsToSelector:@selector(statusCode)] || (((NSHTTPURLResponse *)response).statusCode < 400 && ((NSHTTPURLResponse *)response).statusCode != 304)) {
        ...
        //MCDownloadReceiveResponseNotification 发出通知
    }else if (![response respondsToSelector:@selector(statusCode)] || (((NSHTTPURLResponse *)response).statusCode == 416)) {//Request Range Not Satisfialbe
      //MCDownloadFinishNotification 发出通知
        ...
        //MCDownloadStopNotification 发出通知
