...
@implementation XMGModelOperationTool
...
+ (BOOL)dropTableWithModelClass: (Class)modelClass withUserID: (NSString *)userID {
    ...
    BOOL result = [[XMGSqliteTool shareInstance] dealSQL:sql withUserID:userID];
    return result;
...
+ (BOOL)createTableWithModelClass: (Class)modelClass withUserID: (NSString *)userID {
    ...
    NSArray *array = [[XMGSqliteTool shareInstance] querySQL:existsSql withUserID:userID];
    if (array.count > 0) {
        return YES;
    }
    // 1.1. 查看是否具备主键
...
+ (BOOL)saveModel: (id)model  withUserID: (NSString *)userID {
    ...
    NSArray *result = [[XMGSqliteTool shareInstance] querySQL:hasSql withUserID:userID];
    if (result.count > 0) {
        // 执行更新操作
        ...
        return [[XMGSqliteTool shareInstance] dealSQL:updateSql withUserID:userID];
        ...
        return [[XMGSqliteTool shareInstance] dealSQL:insertSql withUserID:userID];
    }
...
+ (BOOL)deleteModels: (Class)modelClass whereColumnName: (NSString *)column isValue: (id)value  withUserID: (NSString *)userID {
    ...
    return [[XMGSqliteTool shareInstance] dealSQL:deleteSql withUserID:userID];
...
+ (NSArray *)queryModels:(Class)modelClass withCondition: (NSString *)condition withUserID: (NSString *)userID {
    ...
    NSArray *rowDicArray = [[XMGSqliteTool shareInstance] querySQL:querySql withUserID:userID];
...
+ (NSArray *)queryModelsWithSql:(NSString *)sql withUserID:(NSString *)userID {
    ...
    return [[XMGSqliteTool shareInstance] querySQL:sql withUserID:userID];
