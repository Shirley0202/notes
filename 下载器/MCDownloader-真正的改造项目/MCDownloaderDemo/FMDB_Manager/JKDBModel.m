...
- (BOOL)saveOrUpdateByColumnName:(NSString*)sqlieStr
{
    id record = [self.class findFirstByCriteria:[NSString stringWithFormat:@"%@",sqlieStr]];
...
/** 跨表查询 */
+ (NSArray *)findFromTable:(Class) classA toTable:(Class) classB ByCriteria:(NSString *)criteria
{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableA = NSStringFromClass(classA);
        NSString *tableB = NSStringFromClass(classB);
        //select s.name,s.age from t_student s, t_class c where s.class_id = c.id and c.name = ‘0316iOS’;
        
        NSString *sql = [NSString stringWithFormat:@"SELECT * FROM %@ s,%@ c WHERE %@",tableA,tableB,criteria];
        FMResultSet *resultSet = [db executeQuery:sql];
        while ([resultSet next]) {
            JKDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
