...
+ (NSString *)dbPathWithDirectoryName:(NSString *)directoryName
{
    ...
    if (directoryName == nil || directoryName.length == 0) {
        docsdir = [docsdir stringByAppendingPathComponent:@"HOUDA"];
    } else {
    ...
    NSString *dbpath = [docsdir stringByAppendingPathComponent:@"houda.db"];
    return dbpath;
}
...
+ (NSString *)dbPath
{
    ...
    NSString * path = [self dbPathWithDirectoryName:nil];
    NSLog(@"数据库路径 ------- %@",path);
    return path;
}
