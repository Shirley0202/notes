...
- (void)setUrl:(NSString *)url {
    ...
    SSDownloadItem *receipt = [[SSDownloaderManager sharedDownloader] downloadReceiptForURLString:url];
    
    receipt.customFilePathBlock = ^NSString * _Nullable(SSDownloadItem * _Nullable receipt) {
...
//    self.imageView.image = [UIImage imageWithData:[NSData dataWithContentsOfFile:receipt.filePath]];
    ...
    if (receipt.state == SSDownloadStateDownloading || receipt.state == SSDownloadStateWillResume) {
        [self.button setTitle:@"Stop" forState:UIControlStateNormal];
    }else if (receipt.state == SSDownloadStateCompleted) {
    ...
    receipt.downloaderCompletedBlock = ^(SSDownloadItem *receipt, NSError * _Nullable error, BOOL finished) {
        if (error) {
            [self.button setTitle:@"Start" forState:UIControlStateNormal];
...
- (IBAction)buttonAction:(UIButton *)sender {
    ...
    SSDownloadItem *receipt = [[SSDownloaderManager sharedDownloader] downloadReceiptForURLString:self.url];
    if (receipt.state == SSDownloadStateDownloading || receipt.state == SSDownloadStateWillResume) {
        
        [[SSDownloaderManager sharedDownloader] cancel:receipt completed:^{
            [self.button setTitle:@"Start" forState:UIControlStateNormal];
        }];
    }else if (receipt.state == SSDownloadStateCompleted) {
...
- (void)download {
    ...
    [[SSDownloaderManager sharedDownloader] downloadDataWithURL:[NSURL URLWithString:self.url] progress:^(NSInteger receivedSize, NSInteger expectedSize, NSInteger speed, NSURL * _Nullable targetURL) {

    } completed:^(SSDownloadItem *receipt, NSError * _Nullable error, BOOL finished) {
